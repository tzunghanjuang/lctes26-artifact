grep -E "required [0-9]+ cycles" $BASEDIR/pre_synthesis_cleaned/expt-6/real_run.log | awk '{print "Latency (ms) : " $3/200000000*1000}'
grep -E "required [0-9]+ cycles" $BASEDIR/pre_synthesis_cleaned/expt-6/real_run.log | awk '{print "OP/cycle : " 3491339397*2/$3}'
grep -E "required [0-9]+ cycles" $BASEDIR/pre_synthesis_cleaned/expt-6/real_run.log | awk '{print "GOP/s : " 3491339397*2*0.2/$3}'
grep -E "required [0-9]+ cycles" $BASEDIR/pre_synthesis_cleaned/expt-6/real_run.log | awk '{print "DSP efficiency (%) : " 3491339397*2/$3/1216/2*100}'
