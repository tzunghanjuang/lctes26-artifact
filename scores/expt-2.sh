grep -E "required [0-9]+ cycles" $BASEDIR/pre_synthesis_cleaned/expt-2/real_run.log | awk '{print "Latency (ms) : " $3/200000000*1000}'
grep -E "required [0-9]+ cycles" $BASEDIR/pre_synthesis_cleaned/expt-2/real_run.log | awk '{print "OP/cycle : " 15346630656*2/$3}'
grep -E "required [0-9]+ cycles" $BASEDIR/pre_synthesis_cleaned/expt-2/real_run.log | awk '{print "GOP/s : " 15346630656*2*0.2/$3}'
grep -E "required [0-9]+ cycles" $BASEDIR/pre_synthesis_cleaned/expt-2/real_run.log | awk '{print "DSP efficiency : " 15346630656*2/$3/1152/2}'
